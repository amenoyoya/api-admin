        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @if(isset($query))
                    @if (isset($options->slug))
                        {{-- オプションに slug が指定されていれば関連データへルーティング --}}
                        <p><a href="/admin/{{ $options->slug }}/{{ $query->id }}">{{ $query->{$options->label} }}</a></p>
                    @else
            ...
            @else
                ...
                >
                    ...
                    @if(!$row->required && !@$options->required)
        ...
        @elseif($options->type == 'hasMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                @php
                    ...
                    if (isset($options->slug)) {
                        // オプションに slug が指定されていれば関連データへルーティング
                        $selected_ids = $model::where($options->column, '=', $relationshipData->{$options->key})->get()->map(function ($item, $key) {
                            return $item->id;
                        })->all();
                    }
                @endphp
                ...
                @if($view == 'browse')
                    ...
                    @else
                        @if (isset($selected_ids))
                            <p><a href="/admin/{{ $options->slug }}/{{ $selected_ids[0] }}">{{ $string_values }}</a></p>
                        @else
                            <p>{{ $string_values }}</p>
                        @endif
                    @endif
                @else
                    ...
                    @else
                        <ul>
                            @foreach($selected_values as $selected_index => $selected_value)
                                @if (isset($selected_ids))
                                    <li><a href="/admin/{{ $options->slug }}/{{ $selected_ids[$selected_index] }}">{{ $selected_value }}</a></li>
                                @else
                ...
                @if(isset($query))
                    <ul>
                        @foreach($query as $query_res)
                            @if (isset($options->slug))
                                {{-- オプションに slug が指定されていればルーティング --}}
                                <li><a href="/admin/{{ $options->slug }}/{{ $query_res->id }}">{{ $query_res->{$options->label} }}</a></li>
                            @else
        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    if (isset($options->slug)) {
                        // オプションに slug が指定されていれば関連データへルーティング
                        $selected_ids = isset($relationshipData) ? $relationshipData->belongsToMany($options->model, $options->pivot_table, $options->foreign_pivot_key ?? null, $options->related_pivot_key ?? null, $options->parent_key ?? null, $options->key)->get()->map(function ($item, $key) {
                            return $item->id;
                        })->all() : array();
                    }
                @endphp
